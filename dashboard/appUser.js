...
var User = require('../../models/user/accountManager');
...
module.exports = function(app) {
  ...
  var isAuthed = function(req,res,next) {
    User.isUserAuthed(req,function(user) {
      req.user = user;
  ...
  app.post('/login', function(req, res) {
   User.loginUser(req,function(result) {
      if (result.errors) {
  ...
  app.get('/settings', isAuthed, function(req, res) {
    User.findOne({user: req.user.user}, function(e, o) {
  ...
  app.post('/settings', isAuthed, function(req, res) {
    User.findOne({user: req.user.user}, function(e, o) {
      ...
      if (errors.length === 0) {
        ...
        if (settings.newpw==="" && settings.newpwr==="") {
          User.update({_id: o.id}, newData, {upsert: false}, function(err, r) {

         });
       } else {
          User.saltAndHash(settings.newpw, function(hash) {
            newData.pass = hash;
            User.update({_id: o.id}, newData, {upsert: false}, function(err, r) {
           });
         });
  ...
  app.post('/settings/apikey', isAuthed, function(req, res) {
    ...
    User.createApiKey(name, req.user, function(user,newKey) {
      res.json({
        user: user,
        newKey: newKey
     });
  ...
  app.delete('/settings/apikey/:hash', isAuthed, function(req, res) {
    ...
    User.deleteApiKey(req.params.hash, req.user, function(e,r) {
      res.json({e: e,r:r});
   });
 });
}
