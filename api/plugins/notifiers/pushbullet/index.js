...
exports.initWebApp = function() {
  ...
  Event.on('afterInsert', function(event) {
    ...
    event.findCheck(function(err, check) {
      ...
      var message;
      if (event.message === 'up') {
        message = check.name + ' went back up after ' + moment.duration(event.downtime).humanize() + ' of downtime';
     } else {
        message = "The application " + check.name + " just went to status " + event.message
     }
      var msg = {
        message: message,
        title: "Uptime Status",
