...
exports.initWebApp = function(options) {
  ...
  Event.on('afterInsert', function(event) {
    if (!config.event[event.message]) return;
    event.findCheck(function(err, check) {
      if (err) return console.error(err);

      // Pager Duty Direct Start
      pager.create({
        description: "Check " + check.name + " status changed to " + event.message , // required
        details: {
          details: event.details,
          timestamp: moment(event.timestamp).format('LLLL'),
          url: options.config.url + "/dashboard/checks/" + check._id + "?type=hour&date=" + event.timestamp.valueOf()
       },
        callback: function(err, response) {
          if (err) return console.error('PagerDuty plugin error: %s', err);
          console.log('Notified Pagerduty API of error: Check ' + check.name + ' ' + event.message);
       }
     });
   });
 });
